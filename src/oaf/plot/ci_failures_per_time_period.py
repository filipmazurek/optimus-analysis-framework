...
def plot(ci_per_node, time_period=None, filename=None, failure_type='all'):
    """
    Plot the confidence intervals for the number of failures within a time period.

    :param ci_per_node: dict: A dictionary where keys are nodes and values are tuples of the lower and upper bounds of the CI.
    """
    assert time_period is not None, 'Time period must be specified.'

    if failure_type == 'all':
        title = f'Node failure every {time_period} for all failures'
    elif failure_type == 'base':
        title = f'Node failure every {time_period} for base failures'
    else:
    ...
    ax.set_xlabel("Confidence Interval for Failure Rate")
    ax.set_title(title)
...
if __name__ == '__main__':
    ...
    plot(ci_data, time_period='One Day', failure_type='all')
